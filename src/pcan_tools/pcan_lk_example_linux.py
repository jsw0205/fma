...
def parse_args():
    parser = argparse.ArgumentParser(description="LK TC275 SocketCAN Python example")
...
class RawTerminal:
    """Puts stdin into cbreak mode so single keypresses are readable
    without Enter, and restores the original settings on exit."""
...
def read_key():
    ...
    if ch == "\x1b":
        # Escape sequence (arrow/function keys) - drain and ignore.
        for _ in range(2):
            ready, _, _ = select.select([sys.stdin], [], [], 0)
            if ready:
                sys.stdin.read(1)
...
def open_bus(channel, bitrate):
    try:
        ...
        raise SystemExit(
            ...
            f"이미 떠 있다면 'ip -details link show {channel}'로 상태를 확인하세요."
        ) from exc
...
def main():
    ...
    with open_bus(args.channel, args.bitrate) as bus, RawTerminal():
        print("SocketCAN LK terminal control")
